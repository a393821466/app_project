		...
		<view :class="{'pickerMask':showPicker}" @click="maskClick" catchtouchmove="true">
			<view class="r-dtpicker" :class="{'r-dtpicker-show':showPicker}">
				...
				<view class="rangeBox" catchtouchmove="true" @tap.stop="returnHandle">
					<input type="text" disabled placeholder="开始时间" :value="startDate" :style="{color:themeColor,'border-color':themeColor,opacity:dateType=='startDate'?1:.5}"
					 @tap="changeDateType('startDate')">至<input type="text" disabled placeholder="结束时间" :style="{color:themeColor,'border-color':themeColor,opacity:dateType=='endDate'?1:.5}"
...
<script>
	export default {
		...
		created() {
			var that = this;
			if ((this.fields == 'year' && this.start.length != 4) || (this.fields == 'month' && this.start.length != 7) || (this
					.fields == 'day' && this.start.length != 10)) {
