			...
			<view class="setting_top">
				<view class="setting_list">
					<view class="setting_title">
						<input type="text" class="emailView" maxlength="6" placeholder="请输入手机验证码" v-model="user.phoneCode">
					</view>
					<view class="setting">
...
<script>
	...
	export default {
		...
		methods: {
			...mapActions(['getVerifyImg', 'sendVerifyCode', 'setFundsPassword']),
			// 验证码dialog
			openCode() {
				this.$refs.code.open()
				this.verifyCodes()
			...
			setSubmitPwd() {
				...
				if (this.flat) {
					this.flat = false
					that.setFundsPassword(this.user).then(res => {
						this.flat = true
						if (res.status) {
							showUiToast('设置成功');
						} else {
